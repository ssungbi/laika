...
pattern = re.compile(r'\$\(_([A-Z0-9]+)_li\)\.html\([\'"](.*?)hashchange\(\\\'([A-Z0-9]+)\\\'\).*?<span[^>]*>(.*?)</span>(.*?)[\'"]\);')
matches = pattern.findall(js_text)
print(f'Total jQuery categories: {len(matches)}')
for match in matches[:5]: print(match[0], match[3])
