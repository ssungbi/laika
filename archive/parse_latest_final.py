...
def parse_docx_txt(filepath, outpath):
    ...
    while i < len(lines):
        ...
        if '① 총칙' in line:
            ...
            i += 1
            continue
        elif '<부표>' in line or ('일상생활 기본동작(ADLs)' in line and '제한' in line):
        ...
        if current_section == 'chongchik':
            ...
            if m and len(line) < 30 and m.group(1).isdigit():
                ...
                while i < len(lines) and not re.match(r'^(\d+)\.\s+(.*장해.*|.*추상.*)$', lines[i]) and '<부표>' not in lines[i] and '일상생활 기본동작' not in lines[i]:
                    ...
                    items = []
                    rates = []
                    while i < len(lines) and not re.match(r'^나\.\s*장해', lines[i]) and '<부표>' not in lines[i] and '일상생활 기본동작' not in lines[i]:
