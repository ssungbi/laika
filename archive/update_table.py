...
new_code = """// Render comparison table
        ...
        tableHtml += `<th style="padding: 8px; border-right: 1px solid #e2e8f0; text-align: center; width: 20%;">사고상황</th>`;
        detail.cases.forEach((c, idx) => {
            const isLast = idx === detail.cases.length - 1;
            tableHtml += `<th style="padding: 8px; text-align: center; ${!isLast ? 'border-right: 1px solid #e2e8f0;' : ''}">${c.label || '케이스 ' + (idx+1)}</th>`;
        });
        tableHtml += `</tr>`;
        
        // Row 2: 자동차 A
        tableHtml += `<tr style="border-bottom: 1px solid #e2e8f0;">`;
        tableHtml += `<td style="padding: 8px; border-right: 1px solid #e2e8f0; font-weight: bold; background: #f8fafc; text-align: center;">자동차 A</td>`;
        detail.cases.forEach((c, idx) => {
            const isLast = idx === detail.cases.length - 1;
            tableHtml += `<td style="padding: 8px; ${!isLast ? 'border-right: 1px solid #e2e8f0;' : ''}">${c.car_a_text || ''}</td>`;
        });
        tableHtml += `</tr>`;
        
        // Row 3: 자동차 B
        tableHtml += `<tr>`;
        tableHtml += `<td style="padding: 8px; border-right: 1px solid #e2e8f0; font-weight: bold; background: #f8fafc; text-align: center;">자동차 B</td>`;
        detail.cases.forEach((c, idx) => {
            const isLast = idx === detail.cases.length - 1;
            tableHtml += `<td style="padding: 8px; ${!isLast ? 'border-right: 1px solid #e2e8f0;' : ''}">${c.car_b_text || ''}</td>`;
        });
        tableHtml += `</tr>`;
