...
old_sidebar = """<a href="#" class="nav-item">
                        <span class="nav-icon-box">등급</span>
                        <span class="text">등급별 장해표(기타)</span>
                    </a>"""
new_sidebar = """<a href="#" class="nav-item" onclick="navigateTo('view-other-disability'); return false;">
                        <span class="nav-icon-box">등급</span>
                        <span class="text">등급별 장해표(기타)</span>
                    </a>"""
...
# 2. Update Grid Link
old_grid = """<a href="#" class="tool-card-new tc-purple">
                                    <div class="tc-icon material-icons-round">format_list_numbered</div>
                                    <span class="tc-name">등급별 장해표<br>(기타)</span>
                                </a>"""
new_grid = """<a href="#" class="tool-card-new tc-purple" onclick="navigateTo('view-other-disability'); return false;">
                                    <div class="tc-icon material-icons-round">format_list_numbered</div>
                                    <span class="tc-name">등급별 장해표<br>(기타)</span>
                                </a>"""
content = content.replace(old_grid, new_grid)

# 3. Inject View Container
# We'll inject it before <!-- Navigation Script --> or before the closing </div> of .main-content
view_html = """
            <!-- 7. 등급별 장해표(기타) View -->
...
"""
# Inject before Navigation Script or at the end of main-content
if "<!-- 7. 등급별 장해표(기타) View -->" not in content:
    content = content.replace("<!-- Navigation Script -->", view_html + "\n            <!-- Navigation Script -->")

# 4. Inject script tags
scripts_html = """
...
if "other_disability_data.js" not in content:
    content = content.replace("<!-- Navigation Script -->", scripts_html)
