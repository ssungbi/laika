...
print(f"Extracted {len(all_tables)} tables with strict boundaries.")
...
def make_html_table(table_data):
    # center align the table and cells as requested
    ...
    for r_idx, row in enumerate(table_data):
        ...
        for cell in row:
            ...
            if r_idx == 0:
                ...
                # keep '상해내용' (usually longest text) center aligned as requested, but if it's too long it might look better left-aligned. However user specifically asked for center align for the table contents.
                html += f"<td style='border: 1px solid #cbd5e1; padding: 8px; text-align: center;'>{val}</td>"
        html += "</tr>"
    html += "</table></div>"
    return html

# Make sure we don't index out of bounds if there are fewer tables extracted
for item in mapping:
    t_idx = item[0]
    cat = item[1]
    title = item[2]
...
if start_idx != -1:
    ...
    with open(js_path, "w", encoding="utf-8") as f:
        f.write(new_js)
    print("Successfully rebuilt tables and injected into injury_data.js")
else:
