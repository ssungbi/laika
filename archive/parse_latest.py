...
def parse_docx_txt(filepath, outpath):
    ...
    while i < len(lines):
        ...
        if '총칙' in line:
            ...
            i += 1
            continue
        elif '<부표>' in line or ('일상생활 기본동작(ADLs)' in line and '제한' in line) or current_section == 'adls':
            if '<부표>' in line or '일상생활 기본동작(ADLs)' in line:
        ...
        if current_section == 'chongchik':
            ...
            if m and len(line) < 30 and m.group(1).isdigit():
                ...
                while i < len(lines) and not re.match(r'^(\d+)\.\s+(.*장해.*|.*추상.*)$', lines[i]) and '<부표>' not in lines[i]:
                    ...
                    items = []
                    rates = []
                    while i < len(lines) and not re.match(r'^나\.\s*장해', lines[i]) and '<부표>' not in lines[i]:
    ...
    print(f"Items in part 13: {len(parts[12]['items']) if len(parts) > 12 else 0}")
